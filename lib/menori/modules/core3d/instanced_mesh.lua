...
local GeometryBuffer = require ('core3d.geometry_buffer')
...
---@class menori.InstancedMesh : menori.Class
local InstancedMesh = class('InstancedMesh')

local default_format_old = {
...
function InstancedMesh:_attach_buffers()
	...
	for i, v in ipairs(buffer.format) do
		self.lg_mesh:attachAttribute(v[1], buffer.mesh, "perinstance")
	end
end
