...
local Material = require ('core3d.material')
local ffi      = require_o('ffi')

local vec3     = ml.vec3
local mat4     = ml.mat4
local bound3   = ml.bound3

---@class menori.ModelNode : menori.Node
...
---@param scene menori.Scene
---@param environment menori.Environment
...
---@diagnostic disable-next-line: undefined-field
	if self.joints then
		self:recursive_update_transform()
---@diagnostic disable-next-line: undefined-field
		if self.skeleton_node then root_mat:copy(self.skeleton_node.world_matrix)
		else
